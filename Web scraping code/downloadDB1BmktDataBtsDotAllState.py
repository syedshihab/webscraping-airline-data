...
browser.get('https://www.transtats.bts.gov/Tables.asp?DB_ID=125&DB_Name=Airline') # open this website
...
checkboxElem8 = browser.find_element_by_css_selector('input[value="TK_CARRIER_GROUP"]') # Check it
checkboxElem9 = browser.find_element_by_css_selector('input[value="TICKET_CARRIER"]') # Check it
# Uncheck unnecessary attributes
checkboxElem1.click()
checkboxElem2.click()
checkboxElem4.click()
checkboxElem5.click()
# check TK_CARRIER_GROUP, TICKET_CARRIER and PASSENGERS
checkboxElem8.click()
checkboxElem9.click()
checkboxElem7.click()
...
for q in quarter:
    ...
    os.makedirs(os.path.join('C:\\Users\\Syed\\AppData\\Local\\Programs\\Python\\Python36-32\\DB1BmktDataset',quarter2[qq])) # make a new folder for each quarter: Q1, Q2, Q3 and Q4
    
    # browser.execute_script("return arguments[0].scrollIntoView();", webElem2) # Does not work
    for y in year:
        select2.select_by_visible_text(y)
        os.makedirs(os.path.join('C:\\Users\\Syed\\AppData\\Local\\Programs\\Python\\Python36-32\\DB1BmktDataset',quarter2[qq],y))
        folderPath = os.path.join('C:\\Users\\Syed\\AppData\\Local\\Programs\\Python\\Python36-32\\DB1BmktDataset',quarter2[qq],y)
        # for s in state:
